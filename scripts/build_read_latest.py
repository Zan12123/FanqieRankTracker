"""
构建 latest_read_ranks.json（阅读榜）：
1. 加载最近两天的 JSON 快照
2. 按分类对比趋势（新上榜/掉榜/排名变化/阅读量变化）
3. 可选调用 Gemini Flash 生成 AI 总结
4. 输出 latest_read_ranks.json + trends/YYYY-MM-DD.json
"""
import os
import re
import json
...
def build_lastest_api(output: dict, base_dir: str):
    ...
    - api/read-lastest/all.json：全量数据
    - api/read-lastest/<type>.json：单个类型数据
    - api/read-lastest.json / api/read-lastest/index.json：类型索引
    """
    api_root = os.path.join(base_dir, "api")
    lastest_dir = os.path.join(api_root, "read-lastest")
    ...
    for cat in categories:
        ...
        url = f"api/read-lastest/{quote(filename)}.json"
        types.append({
            "type": type_name,
            "url": url,
    ...
    write_json(os.path.join(api_root, "read-lastest.json"), index_payload)
...
def build_market_ai_prompt(payload: dict) -> str:
    ...
    return f"""你是一位网文市场编辑，请根据番茄女频阅读榜的统计结果，为每个周期生成一段全站热点判断。
...
def main():
    parser = argparse.ArgumentParser(description="构建 latest_read_ranks.json")
    ...
    # 查找 JSON 快照文件
    snapshots = sorted(
        glob.glob(os.path.join(data_dir, "fanqie_female_read_ranks_*.json"))
    )
    ...
    if args.date:
        ...
        target_path = os.path.join(
            data_dir, f"fanqie_female_read_ranks_{target_date_compact}.json"
        )
    ...
    # 写入 latest_read_ranks.json
    out_path = os.path.join(data_dir, "latest_read_ranks.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"\n✅ 已生成: {out_path}")

    # 生成静态 API 文件：api/read-lastest/all.json + api/read-lastest/<type>.json
    ...
    market_path = os.path.join(data_dir, "market_read_summary.json")
    ...
    for s in snapshots:
        ...
        # fanqie_female_read_ranks_YYYYMMDD.json -> YYYY-MM-DD
