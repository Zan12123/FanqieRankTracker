...
def run_scraper(limit=30, sleep_sec=5):
    ...
    output_file = os.path.join(OUTPUT_DIR, f"fanqie_female_read_ranks_{date_str}.json")
    state_file = os.path.join(OUTPUT_DIR, f"task_state_read_{date_str}.json")
    ...
    with sync_playwright() as p:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 正在初始化并访问基础榜单页（阅读榜）：{init_url}")
