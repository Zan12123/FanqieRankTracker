document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`data/dates.json?${cacheBuster}`)
        ...
        .then(idx => {
            ...
            // Start by loading latest_read_ranks.json (already has trend data baked in)
    ...
    function loadLatestData() {
        return fetch(`data/latest_read_ranks.json?${cacheBuster}`)
            .then(r => {
    ...
    function loadDateData(dateStr) {
        ...
        Promise.all([
            ...
            // Build a data object in the same shape as latest_read_ranks.json
            const combined = {
                date: snapshot.date,
